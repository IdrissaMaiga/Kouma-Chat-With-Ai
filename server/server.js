...
const SERVER_URL = "http://localhost:3000";       //"https://server.filmutunnel.site";
...
app.get('/get-openai-response', async (req, res) => {
  const { message } = req.query; // Use query parameters instead of req.body
  ...
  try {
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        { role: 'system', content: 'You are an assistant called Kouma that can answer anything well.' },
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', ({ roomId, username }) => {
    ...
    // Update user list with unique usernames
const roomUsers = Array.from(
  new Set( // Use Set to ensure unique usernames
    Array.from(io.sockets.adapter.rooms.get(roomId) || []).map(
      (id) => io.sockets.sockets.get(id)?.username
    )
  )
).filter(Boolean); // Remove undefined/null usernames, if any
